...
public sealed class WorldEntityRegistryTests
{
    ...
    {
        var registry = new WorldEntityRegistry();
    ...
    [Fact]
    public void ReconnectPreservesEntityIdAndMovementForTheSameWorldSession()
    {
        var registry = new WorldEntityRegistry();
        ...
        {
            WorldSessionToken = "rotated-session-token",
        ...
        Assert.Equal("rotated-session-token", reconnect.Entity.Session.WorldSessionToken);
        Assert.Equal(1u, reconnect.Entity.Movement.LastProcessedInputSequence);
        Assert.Equal(0, movementFactoryCalls);
    }

    [Fact]
    public void NewWorldSessionReplacesTheCharactersPreviousEntity()
    {
        var registry = new WorldEntityRegistry();
        var originalSession = CreateSession();
        var original = registry.RegisterPlayer(originalSession, CreateMovement);
        var replacementSession = originalSession with
        {
            ...
            WorldSessionToken = "replacement-session-token"
        };
    ...
    [Fact]
    public void RemoveRequiresTheExactWorldSession()
    {
        var registry = new WorldEntityRegistry();
        ...
        Assert.True(registry.RemovePlayer(
            registration.Entity.NetworkEntityId,
            registration.Entity.Session.WorldSessionId,
            out var removed));
